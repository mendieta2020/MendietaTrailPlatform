"""
Wahoo OAuth integration provider — STUB (disabled).

Status: Coming Soon — not yet enabled for production.

Notes:
- Wahoo uses OAuth 2.0 authorization code flow (Wahoo Cloud API).
- Requires WAHOO_CLIENT_ID + WAHOO_CLIENT_SECRET in settings.
- API docs: https://cloud-api.wahooligan.com/
"""
...
class WahooProvider(IntegrationProvider):
    """
    Wahoo integration provider stub.
    ...
    def get_oauth_authorize_url(self, state: str, callback_uri: str) -> str:
        raise NotImplementedError(
            "WahooProvider is not yet implemented. "
