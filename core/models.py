...
class AthleteDevicePreference(models.Model):
    """
    Records whether an athlete has dismissed the device-connection prompt.

    dismissed=True means the athlete chose "No tengo dispositivo" and should not
    be prompted automatically again. Reversible: the athlete can reactivate from
    their profile (sets dismissed=False).

    Tenancy: organization FK is non-nullable. unique_together enforces one record
    per (organization, athlete).
    """

    DISMISSED_REASON_CHOICES = [
        ("no_device", "No tiene dispositivo"),
    ]

    organization = models.ForeignKey(
        "Organization",
        on_delete=models.CASCADE,
        related_name="athlete_device_preferences",
    )
    athlete = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
    ...
    dismissed_reason = models.CharField(
        max_length=20,
        choices=DISMISSED_REASON_CHOICES,
        null=True,
        blank=True,
    )
    ...
    def __str__(self):
        return f"DevicePref: athlete={self.athlete_id} org={self.organization_id} dismissed={self.dismissed}"


# ==============================================================================
# PR-141: AthleteNotification — internal notifications (coach → athlete)
# ==============================================================================

class AthleteNotification(models.Model):
    """
    Internal notification from a coach (or the system) to an athlete.

    notification_type='device_connect': coach is asking the athlete to connect
    their training device. Duplicate guard: only one unread notification of the
    same type per (recipient, organization) should exist at any time.

    sender=None means the notification was generated by the system (auto-prompt).
    Tenancy: organization FK is non-nullable.
    """
    ...
    organization = models.ForeignKey(
        "Organization",
        on_delete=models.CASCADE,
        related_name="athlete_notifications",
    )
    ...
    sender = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="sent_athlete_notifications",
    )
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["recipient", "organization", "read"]),
        ]
...
# ==============================================================================
# PR-154: AthleteReport — shareable training report (WhatsApp + email)
# ==============================================================================

class AthleteReport(models.Model):
    """
    A point-in-time snapshot of an athlete's training metrics, generated by a
    coach and shared via a public tokenized URL (no login required).

    The snapshot field captures all data at generation time so the report
    remains stable even if underlying data changes.

    Token expires after 7 days. View count tracks how many times the athlete
    opened the link.

    Tenancy: organization FK is non-nullable; all access validated via membership.
    """
    ...
    # Stable snapshot of data at generation time
